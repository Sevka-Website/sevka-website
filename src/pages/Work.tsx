...
const projects = [
  {
    id: 'claradent', title: 'ClaraDent Dental Clinic', category: 'Corporate Sites',
    image: '/claradent-thumb.jpg', tags: ['Healthcare', 'Booking System', 'Landing Page'],
    ...
    route: '/work/claradent', year: '2025', transitionTitle: 'ClaraDent', transitionSubtitle: 'Dental Clinic', transitionIndex: '01',
  },
  {
  ...
  return (
    <>
      <AnimatePresence>
        {activeTransition && (
          <ProjectTransition
            project={{ title: activeTransition.transitionTitle, subtitle: activeTransition.transitionSubtitle, year: activeTransition.year, index: activeTransition.transitionIndex, route: activeTransition.route }}
      ...
      <div className="flex flex-col w-full min-h-screen">
        <section className="pt-32 pb-20 px-6 max-w-7xl mx-auto w-full">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <p className="text-xl text-muted-warm max-w-2xl mb-16">A selection of websites, stores, and experiences we've built for clients worldwide.</p>
          </motion.div>
          ...
          <motion.div layout className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.1 }} key={project.id}
                ...
                <div className="p-8 flex flex-col gap-4 flex-grow">
                  <div className="flex flex-wrap gap-2">
                    ...
                    {project.tags.map(tag => <span key={tag} className="px-3 py-1 border border-white/10 rounded-full text-xs font-mono text-warm-white/50 uppercase tracking-wider">{tag}</span>)}
                  </div>
                  ...
                  <button onClick={() => setActiveTransition(project)} className="inline-flex items-center gap-2 text-gold font-semibold hover:gap-4 transition-all w-fit mt-2 group/btn cursor-pointer">
                    View Live Project <ArrowRight size={18} className="transition-transform group-hover/btn:translate-x-1" />
                  </button>
                </div>
              </motion.div>
            ))}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.3 }}
              ...
              <div className="flex flex-col items-center text-center gap-4">
                <div className="w-16 h-16 rounded-full border border-gold/20 flex items-center justify-center"><span className="text-gold text-2xl font-light">+</span></div>
                <h3 className="font-display text-xl font-bold text-warm-white/40">More Coming Soon</h3>
                <p className="text-muted text-sm max-w-xs">New projects are added as they launch.</p>
              </div>
            </motion.div>
          </motion.div>
        </section>
        <section className="py-24 px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="max-w-2xl mx-auto">
            ...
            <p className="text-muted-warm mb-10">We're selective about the projects we take on — every client gets our full attention.</p>
